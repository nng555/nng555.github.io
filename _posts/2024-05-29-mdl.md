...
*This is part 1 of a blog post on our [**ICML 2024 paper**](/assets/files/stochastic_complexity.pdf). 
...
If our model is parametric, then we incur a constant cost $|\theta|$ to transmit the model to Alice, and minimizing the codelength reduces to finding a set of parameters $\hparam$ that minimizes $\cost_{\hparam}(\X, \Y)$ well.
The model $\hparam$ will then have successfully compressed and learned the data.

Notice that the codelength corresponds to the log loss, and the objective above should look very familiar as the standard maximum likelihood estimation (MLE) objective.
