...
Given some vector space $\H$ over $\RR$, a function $\langle \cdot, \cdot \rangle_\H : \H \times \H \rightarrow \RR$ is an inner product if it satisfies linearity in the first term, symmetry, and positive definiteness ($\langle f, f \rangle > 0$, and $\langle f, f\rangle = 0 \Leftrightarrow f = \mathbf{0}$).
